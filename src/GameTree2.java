...
public class GameTree2<Move> {
	  ...
	  public GameTree2(Board2<Move> board, 
			  Set<Move> moves, 
	                  int optimalOutcome, Move lastMove) {
	...
	this.optimalOutcome = optimalOutcome;
	  ...
	  public void firstPlayer(MoveChannel<Move> c, Computer comp) {
	    ...
	    if (isLeaf()) {
	      ...
	      c.end(board.value());
	    	...
	    	GameTree2<Move> tree = board.tree( Integer.MIN_VALUE, Integer.MAX_VALUE,c, comp);
	    	c.giveMove(lastMove);
	    	GameTree2<Move> newTree = board.tree(tree.lastMove);
	    	newTree.secondPlayer(c, comp);
	    }
	  }

	  // Plays second using this tree:
	  public void secondPlayer(MoveChannel<Move> c, Computer comp) {
	    c.comment(board + "\nThe optimal outcome is " + optimalOutcome);
